...
import { Cog6ToothIcon, UsersIcon, DocumentChartBarIcon } from './icons';
...
const Settings: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'general' | 'operators' | 'data'>('general');
                    ...
                    <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                        ...
                        >
                         ...
                         <button
                            ...
                            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center gap-2 ${activeTab === 'data' ? 'border-brand-blue text-brand-blue' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                        >
...
const GeneralSettings = () => {
    ...
    const [defaultLateTime, setDefaultLateTime] = useState<string>('07:30');
    const [lateTimeFeedback, setLateTimeFeedback] = useState<Feedback | null>(null);
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            ...
            const lateTime = await dataService.getSetting<string>('defaultLateTime');
            if (lateTime) setDefaultLateTime(lateTime);
        };
        fetchSettings();
    }, []);
    ...
    const handleLateTimeChange = async () => {
        try {
            await dataService.updateSetting('defaultLateTime', defaultLateTime);
            showFeedback(setLateTimeFeedback, 'Jam batas terlambat default berhasil disimpan.', 'success');
        } catch (error) {
             console.error(error);
             showFeedback(setLateTimeFeedback, 'Gagal menyimpan jam batas terlambat.', 'error');
        }
    }
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
             ...
             {/* Late Time Setting */}
            <div className="bg-white p-6 rounded-lg shadow-md">
                 <h2 className="text-xl font-bold mb-6 text-gray-800">Pengaturan Jam Masuk</h2>
                 {lateTimeFeedback && (
                    <div className={`p-4 mb-4 text-sm rounded-lg ${lateTimeFeedback.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {lateTimeFeedback.message}
                    </div>
                )}
                <div className="space-y-4">
                    <div>
                        <label htmlFor="defaultLateTime" className="block text-sm font-medium text-gray-700 mb-1">
                            Jam Batas Terlambat (Default)
                        </label>
                        <p className="text-sm text-gray-500 mb-2">
                            Jam ini akan digunakan sebagai acuan otomatis saat Operator membuka halaman Scan Absen.
                        </p>
                        <div className="flex items-center gap-4">
                             <input
                                ...
                                className="block w-40 px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-blue focus:border-brand-blue"
                            />
                            <button
                                type="button"
                                onClick={handleLateTimeChange}
                                className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-brand-blue hover:bg-brand-blue-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-blue"
                            >
                                Simpan
                            </button>
...
const DataSettings = () => {
    ...
    const handleClearAttendance = async () => {
        ...
        setIsLoading(true);
        try {
            await dataService.clearAttendanceData();
            setFeedback({ message: 'Seluruh data absensi berhasil dihapus.', type: 'success' });
            setConfirmText('');
        } catch (error) {
            console.error(error);
            setFeedback({ message: 'Gagal menghapus data absensi.', type: 'error' });
        } finally {
            setIsLoading(false);
            setTimeout(() => setFeedback(null), 3000);
        }
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-bold mb-6 text-gray-800">Manajemen Data Absensi</h2>
            ...
            <div className="border border-red-200 bg-red-50 p-6 rounded-lg">
                <h3 className="text-lg font-bold text-red-700 mb-2">Hapus Seluruh Data Absensi</h3>
                <p className="text-sm text-red-600 mb-4">
                    Tindakan ini akan <strong>menghapus permanen</strong> seluruh riwayat absensi siswa. Data siswa, guru, dan pengaturan sekolah <strong>tidak</strong> akan terhapus. Gunakan fitur ini saat pergantian semester atau tahun ajaran baru.
                </p>
                ...
                <div className="mb-4">
                    ...
                    <input
                        ...
                        className="block w-full max-w-xs px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500"
                    />
                </div>

                <button
                    onClick={handleClearAttendance}
                    disabled={confirmText !== 'HAPUS SEMUA' || isLoading}
                ...
                >
                    {isLoading ? 'Memproses...' : 'Hapus Data Absensi'}
                </button>
            </div>
        </div>
    );
