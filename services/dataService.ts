...
const seedInitialData = async () => {
    ...
    if (!passwordSetting) {
        ...
        // Default late time setting
        await promisifyRequest(settingsStore.put({ key: 'defaultLateTime', value: '07:30' }));
        
        console.log('Settings seeding complete.');
    } else {
        // Ensure late time setting exists even if other settings exist
        const lateTime = await promisifyRequest(settingsStore.get('defaultLateTime'));
        if (!lateTime) {
             await promisifyRequest(settingsStore.put({ key: 'defaultLateTime', value: '07:30' }));
        }
    }
...
export const dataService = {
    ...
    clearAttendanceData: async (): Promise<void> => {
        const db = await getDb();
